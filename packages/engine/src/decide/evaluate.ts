...
export function evaluateDecisions(input: DecisionEvaluationInput): DecideResult {
  ...
  while (true) {
    ...
    if (visited.has(currentDecisionId)) {
      throw new DecisionEvaluationError("decide.cycle", "The decision path contains a cycle.");
    }
...
function findMatchingBranch(
  ...
  try {
    for (const branch of decision.branches) {
      conditions.push(parseCondition(branch.when));
    }
  } catch (error) {
  ...
  for (let index = 0; index < decision.branches.length; index += 1) {
    if (evaluateParsedCondition(conditions[index]!, context)) return decision.branches[index];
  }
  return undefined;
}
