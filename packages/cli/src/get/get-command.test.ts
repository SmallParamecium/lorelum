...
test.each([
  ["--format=json", ["get", practice.id, "--format=json"]],
  ["--json", ["--json", "get", practice.id]],
] as const)(
...
test.each([["--format=text"], ["--human"]] as const)(
  "text selector %s returns only the complete Practice body",
  async (selector) => {
    const result = await invokeText(["get", practice.id, selector], {
      async getEffectivePractice() {
        return effective;
      },
    });
...
test("text get preserves a Practice body without a final line feed", async () => {
  ...
  const result = await invokeText(["get", practice.id, "--format=text"], {
    async getEffectivePractice() {
      return {
        ...effective,
...
test("text get preserves CRLF and repeated trailing line feeds verbatim", async () => {
  ...
  const result = await invokeText(["get", practice.id, "--human"], {
    async getEffectivePractice() {
      return {
        ...effective,
        practice: { ...effective.practice, body },
...
test("text get failures go to stderr without changing the exit code", async () => {
  const result = await invokeText(["get", "sample.absent", "--format=text"], {
    async getEffectivePractice() {
      return undefined;
    },
  });
