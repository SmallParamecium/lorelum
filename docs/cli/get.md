...
Separate invocations can observe different Store revisions; there is no cross-command snapshot or version-pinning contract. A single point read is consistent, but it is not a long-lived Session and does not pin a revision for a later command. In particular, the returned `packRoot` can become stale after a Pack mutation. Refresh it through `get` or `pack list`; do not construct artifact paths from Pack names, SQLite rows, projections, or Store layout.
